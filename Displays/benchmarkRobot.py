...
class Agent(object):
    ...
    def stepController(self):
        ...
        if len(self.params) != getDimensions(): # vérifie que le nombre de paramètres donné est correct
            print ("[ERROR] number of parameters is incorrect.","Given",len(self.params),"- should be:",getDimensions(),"-- Exiting.")
            print (self.params)
            #time.sleep(5)
            running = False
            return ("error")
...
def evaluate(x,eval=1):
    ...
    fitness = 0
    i = 0
    while i < eval: # N évaluations indépendantes
        fitness += agents[0].evaluate(x)
        i = i + 1
    return fitness
